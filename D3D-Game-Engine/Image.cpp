...
Image::Image(std::string filename) : textureView(nullptr), width(0), height(0) {
	// cria sharer resource view da imagem em disco

	D3D11CreateTextureFromFile(
		Graphics::device,				// dispositivo do direct3D
		Graphics::context,				// contexto do dispositivo direct3D
		filename.c_str(),				// nome do arquivo da imagem
...
Image::~Image() {
	// libera a memoria ocupada pela texture view
	if (textureView) {
		// pega ponteiro para o recurso
		ID3D11Resource * resource = nullptr;
		textureView->GetResource(&resource);

		// liberando a view não libera automaticamente
		// o recurso que foi criado junto a view
		if (resource) {
			resource->Release();
			resource = nullptr;
		}
